"""Baseline ResNet1D — patient-grouped 80/20 + 3-fold CV.

Pipeline (matches notebooks/balancing_augmentation.ipynb):
  ...
  3. Preprocess (BWF 0.5 Hz HP + Notch 60 Hz + Z-score per lead) on all data
  4. Patient-grouped 3-fold CV on the 80% dev set
       (~92 positives per val fold — more reliable per-fold AUROC than 5-fold's ~55)
       - On each TRAIN fold only:
           a. GaussianNoise copy of positives → concat
           b. RandomTimeShift copy of positives → concat   (positives now 3×)
           c. MajorityUndersampling (1:1)                  (training set balanced)
       - val fold: preprocessed only
  5. Final retrain on all dev (with same augment+balance pipeline) → score on 20% test
  6. Test report: AUROC, AUPRC (with bootstrap 95% CIs), Brier score,
     operating points at τ=0.5 / Youden / sens≥0.90, confusion + sens/spec/PPV/NPV.

Usage:
    python -m src.train_baseline_resnet1d
    python -m src.train_baseline_resnet1d --data-dir "data/seniordesign_upload"
    python -m src.train_baseline_resnet1d --n-folds 3 --epochs 50
"""
...
from src.models.baseline_resnet1d import BaselineResNet1D, BaselineResNet1DModel
...
PARAMS = dict(
    ...
    weight_decay    = 1e-4,
    batch_size      = 32,
    loss_fn         = "cross_entropy",   # train set is 1:1 after undersample
    patience        = 10,
)
...
def run_cv(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = BaselineResNet1DModel(**PARAMS, epochs=epochs)
...
def sensitivity_threshold(y_true: np.ndarray, probs: np.ndarray, target_sens: float = 0.90) -> float:
    """Lowest threshold achieving at least target_sens. Returns 0.0 if not achievable."""
    fpr, tpr, thresholds = roc_curve(y_true, probs)
    valid = tpr >= target_sens
    if not valid.any():
        return 0.0
    # roc_curve returns thresholds in descending order; prefer the highest threshold meeting the target
...
def render_architecture(net_params: dict, run_dir: Path) -> str:
    """Print a layer-by-layer summary and export ONNX for interactive viewing.

    Returns the text summary so it can be saved alongside other artifacts.
    Drag the .onnx file into https://netron.app for a clickable graph.
    """
    model = BaselineResNet1D(**net_params).eval()
    ...
    try:
        ...
        text = (
            ...
            f"(install `torchinfo` for layer-wise shape & MAC table)"
        )
    ...
    onnx_path = run_dir / "baseline_resnet1d.onnx"
    try:
        torch.onnx.export(
...
def main():
    parser = argparse.ArgumentParser(description="Baseline ResNet1D — patient-grouped CV")
    parser.add_argument("--data-dir", default="data/seniordesign_upload")
    parser.add_argument("--n-folds",  type=int, default=3)
    parser.add_argument("--epochs",   type=int, default=50)
    args = parser.parse_args()

    run_dir = Path("cv_results") / f"baseline_resnet1d_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    # 0. Render architecture (live summary + ONNX export)
    net_params = {k: PARAMS[k] for k in
                  ("stem_channels", "block_channels", "block_strides",
                   "kernel_size", "stem_kernel", "dropout")}
    ...
    print(f"\n{'#'*60}\n  Baseline ResNet1D — {args.n_folds}-fold patient-grouped CV\n{'#'*60}")
    ...
    torch.save(final_model.model.state_dict(), run_dir / "model_baseline_resnet1d.pt")
    ...
    try:
        ...
        fig.update_layout(title="Baseline ResNet1D — Training Curves",
