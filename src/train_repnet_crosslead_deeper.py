"""RepNet CrossLead Deeper — 3-stage variant, patient-grouped 80/20 + 3-fold CV.

Same pipeline as src/train_repnet_crosslead.py, but with the 3-stage
RepNetCrossLeadDeeper model (kernels 7, 5, 3 / channels 32, 64, 128).

Usage:
    python -m src.train_repnet_crosslead_deeper
    python -m src.train_repnet_crosslead_deeper --n-folds 5 --epochs 50
"""
...
PARAMS = dict(
    stage_filters  = (32, 64, 128),     # extends original (32, 64) by doubling once more
    kernels        = (7, 5, 3),         # extends original (7, 5) with the next narrower kernel
    dropout        = 0.0636,            # optuna 2026-04-17 best (transferred from 2-stage)
    n_heads        = 4,
    lr             = 8.76e-4,           # optuna 2026-04-17 best (transferred)
...
def render_architecture(net_params: dict, run_dir: Path) -> str:
    ...
    print("\n" + "=" * 60)
    print("  Model architecture — RepNet CrossLead Deeper (3-stage)")
    print("=" * 60)
    print(text)

    (run_dir / "architecture.txt").write_text(text, encoding="utf-8")

    onnx_path = run_dir / "repnet_crosslead_deeper.onnx"
    try:
        torch.onnx.export(
...
def main():
    parser = argparse.ArgumentParser(description="RepNet CrossLead Deeper — 3-stage CV")
    ...
    print(f"\n{'#'*60}\n  RepNet CrossLead Deeper — {args.n_folds}-fold patient-grouped CV\n{'#'*60}")
    ...
    torch.save(final_model.model.state_dict(), run_dir / "model_repnet_crosslead_deeper.pt")
    ...
    try:
        ...
        fig.update_layout(title="RepNet CrossLead Deeper — Training Curves",
