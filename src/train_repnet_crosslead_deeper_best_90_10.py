"""RepNet CrossLead Deeper — most performant Optuna config, 90/10 test split.

Uses the single most performant config across all 3-stage Optuna studies:
the filter-search best (test AUROC 0.7952), combined with the best params
from the lr+dropout, reg, and RF studies.

Source studies (latest of each):
  optuna_crosslead_3stage         /2026-04-26_23-12-14   lr+dropout
  optuna_crosslead_3stage_reg     /2026-04-27_05-38-28   weight_decay+aug
  optuna_crosslead_3stage_rf      /2026-04-27_08-27-21   kernels
  optuna_crosslead_3stage_filters /2026-04-27_10-29-59   stage_filters  (test best)

Differences from train_repnet_crosslead_deeper_optimized.py:
  - test_size = 0.10 (90/10 split) instead of 0.20 (80/20)
    → ~10% more training data per fold, smaller test set (~25 PE samples).
       Use bootstrap CIs on the test report — they'll be wider with N≈218.
  - No early stopping (full epochs, restore best-val weights at end)

Usage:
    python -m src.train_repnet_crosslead_deeper_best_90_10
    python -m src.train_repnet_crosslead_deeper_best_90_10 --n-folds 5 --epochs 60
"""
...
# Most performant Optuna config (filter study winner, paired with all other bests).
PARAMS = dict(
    stage_filters  = (48, 96, 192),     # filter study best, ~965K params
    kernels        = (7, 5, 3),         # RF study best (RF=180 ms)
    n_heads        = 4,
    lr             = 2.465e-3,
    dropout        = 0.0546,
...
# ----- model wrapper without early stopping -----

class _NoEarlyStopDeeperModel(RepNetCrossLeadDeeperModel):
    """RepNetCrossLeadDeeperModel that trains the full epoch budget (no break).

    Best-val-AUROC weights are still restored at the end.
    """
...
# ----- pipeline (matches optimized trainer) -----
...
def main():
    parser = argparse.ArgumentParser(
        description="RepNet CrossLead Deeper - most performant config, 90/10 split"
    )
    ...
    run_dir = Path("cv_results") / f"repnet_crosslead_deeper_best_90_10_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    print(f"\n{'#'*60}\n  Most performant config - 90/10 - {args.n_folds}-fold CV\n{'#'*60}")
    ...
    torch.save(final_model.model.state_dict(), run_dir / "model_repnet_crosslead_deeper_best.pt")
    ...
    summary = "\n".join([
        f"\n{'='*60}",
        f"  RepNet CrossLead Deeper - most performant Optuna config (90/10)",
        f"{'='*60}",
