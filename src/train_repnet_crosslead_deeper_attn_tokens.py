"""RepNet CrossLead Deeper — best Optuna config + multi-token cross-lead attention.

Same architecture/training setup as train_repnet_crosslead_deeper_best_90_10.py,
with one change: attn_tokens=(8, 4, 1) instead of the default (1, 1, 1).

Motivation: at stage 1 with n_tokens=1, each lead is collapsed to a single
average over 1,250 timesteps before attention runs — so attention sees only
"per-lead average activation," which is dominated by patient-specific amplitude
and is class-agnostic for PE. Giving stage 1 eight temporal tokens lets attention
mix across (lead, time-segment) pairs, e.g. T-wave of V4 vs QRS of II.
Stage 2 gets 4 tokens; stage 3 stays at 1 (deepest stage already integrates
context through its receptive field).

Usage:
    python -m src.train_repnet_crosslead_deeper_attn_tokens
    python -m src.train_repnet_crosslead_deeper_attn_tokens --n-folds 5 --epochs 60
    python -m src.train_repnet_crosslead_deeper_attn_tokens --attn-tokens 8 4 1
"""
...
# Best Optuna config (filter study winner) — unchanged from best_90_10.
PARAMS = dict(
...
DEFAULT_ATTN_TOKENS = (8, 4, 1)
AUG_SIGMA           = 0.060
AUG_MAX_SHIFT       = 276


class _NoEarlyStopDeeperModel(RepNetCrossLeadDeeperModel):
    """Train the full epoch budget; restore best-val-AUROC weights at the end."""
...
def run_cv(X_dev, y_dev, folds, epochs, attn_tokens):
    aurocs, auprcs = [], []
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = RepNetCrossLeadDeeperModel(
            **PARAMS, attn_tokens=attn_tokens, epochs=epochs,
        )
...
def train_final(X_dev, y_dev, g_dev, epochs, seed, attn_tokens):
...
def main():
    parser = argparse.ArgumentParser(
        description="RepNet CrossLead Deeper - best config with multi-token cross-lead attention"
    )
    parser.add_argument("--data-dir", default="data/seniordesign_upload")
    parser.add_argument("--n-folds", type=int, default=5)
    parser.add_argument("--epochs",  type=int, default=50)
    parser.add_argument(
        "--attn-tokens", type=int, nargs=3, default=list(DEFAULT_ATTN_TOKENS),
        metavar=("S1", "S2", "S3"),
        help="Per-stage cross-lead attention tokens (default: 8 4 1).",
    )
    args = parser.parse_args()

    attn_tokens = tuple(args.attn_tokens)
    tag = "x".join(str(t) for t in attn_tokens)
    run_dir = Path("cv_results") / (
        f"repnet_crosslead_deeper_attn{tag}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    )
    ...
    logger.info("attn_tokens (per stage): %s", attn_tokens)
    logger.info("Augmentation: sigma=%.4f  max_shift=%d", AUG_SIGMA, AUG_MAX_SHIFT)
    logger.info("Test split: %.0f/%.0f", 100 * (1 - TEST_SIZE), 100 * TEST_SIZE)
    ...
    print(f"\n{'#'*60}\n  RepNet CrossLead Deeper - attn_tokens={attn_tokens} - 90/10 - {args.n_folds}-fold CV\n{'#'*60}")
    cv_aurocs, cv_auprcs = run_cv(X_dev, y_dev, folds, epochs=args.epochs, attn_tokens=attn_tokens)

    print("\n  Retraining on full dev set ...")
    final_model = train_final(
        X_dev, y_dev, g_dev, epochs=args.epochs, seed=SEED, attn_tokens=attn_tokens,
    )
    probs_test = final_model.predict_proba(X_test)
    torch.save(final_model.model.state_dict(), run_dir / f"model_attn{tag}.pt")
    ...
    summary = "\n".join([
        f"\n{'='*60}",
        f"  RepNet CrossLead Deeper - attn_tokens={attn_tokens} (90/10)",
        f"{'='*60}",
