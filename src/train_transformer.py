"""RepNet Transformer — 80/20 + 5-fold CV evaluation.
...
def main():
    ...
    params = {**TRANSFORMER_PARAMS, "epochs": args.epochs}

    # 5-fold CV
    print(f"\n{'#'*60}")
    print(f"  RepNet Transformer — {args.n_folds}-fold CV")
    print(f"{'#'*60}")
    aurocs = []
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = RepNetTransformerModel(**params)
