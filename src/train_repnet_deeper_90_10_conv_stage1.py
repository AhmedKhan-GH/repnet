"""Most-performant CrossLead Deeper, 90/10 split, **stage 1 conv mix instead of attention**.

Same config as `train_repnet_crosslead_deeper_best_90_10.py` but the stage-1
CrossLeadAttention is replaced by a `LeadMixConv1x1` — a learnable linear mix
across the 12 leads with fixed weights (no per-sample attention dynamics).

Rationale: stage-1 attention was diagnosed as class-agnostic noise. A simple
fixed cross-lead linear mix may still allow useful information to flow between
leads at low feature depth, without the optimization noise of attention.

Architecture at stage 1:
  PerLeadConvBlock(1 -> 48, k=7) -> LeadMixConv1x1(12 leads)   [no attention]
Stages 2 & 3 keep their CrossLeadAttention.

Usage:
    python -m src.train_repnet_deeper_90_10_conv_stage1
    python -m src.train_repnet_deeper_90_10_conv_stage1 --epochs 60 --n-folds 5
"""
...
# attn_stages set to all-True so stage 1 starts with attention; we'll then
# REPLACE stage 1's attention with LeadMixConv1x1 inside the fit() override.
PARAMS = dict(
...
# ------------------------------------------------------------------ #
# Cross-lead linear mix used in place of attention at stage 1.
# Same I/O shape as CrossLeadAttention so it slots into the stage's
# ModuleDict without any other code changes.
# ------------------------------------------------------------------ #

class LeadMixConv1x1(nn.Module):
    """Linear mix across the lead dimension (residual).

    Each output lead is a learned linear combination of input leads, applied
    identically at every (channel, timestep) position. ~144 weights for 12
    leads — much cheaper than CrossLeadAttention.
    """
...
class _ConvStage1DeeperModel(RepNetCrossLeadDeeperModel):
    """Stage-1 attention replaced by LeadMixConv1x1. Early-stop patience=10."""

    def fit(self, X_train, y_train, X_val, y_val):
        self.model = RepNetCrossLeadDeeper(**self.net_params).to(self.device)

        # Replace stage 1's attention with a cross-lead linear mix.
        n_leads = 12
        self.model.stages[0]["attn"] = LeadMixConv1x1(
            n_leads=n_leads, dropout=self.net_params.get("dropout", 0.0),
        ).to(self.device)
...
def run_cv(X_dev, y_dev, folds, epochs):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = _ConvStage1DeeperModel(**PARAMS, epochs=epochs)
...
def main():
    parser = argparse.ArgumentParser(
        description="CrossLead Deeper - 90/10 split - stage 1 attention replaced by LeadMixConv1x1"
    )
    ...
    run_dir = Path("cv_results") / f"deeper_90_10_conv_stage1_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    logger.info("Variant: stage 1 attention REPLACED by LeadMixConv1x1")
    ...
    print(f"\n{'#'*60}\n  90/10 - stage 1 conv mix - {args.n_folds}-fold CV\n{'#'*60}")
    ...
    summary = "\n".join([
        f"\n{'='*60}",
        f"  CrossLead Deeper - 90/10 - stage 1 conv mix (no attention)",
        f"{'='*60}",
