"""lr+dropout re-tuning for the (48, 96, 192) filter-search winner.

The original lr+dropout study was run on the smaller (32, 64, 128) architecture.
Filter study then picked (48, 96, 192) using those same lr/dropout values. With
2.2× more parameters, the optimizer settings might no longer be optimal — bigger
models often want slightly lower lr or different dropout.
...
-------------------------------
  Architecture:   stage_filters=(48, 96, 192), kernels=(7, 5, 3), n_heads=4
                  attn_stages=(True, True, True), attn_tokens=1
  Regularization: weight_decay=1.67e-4
  Augmentation:   aug_sigma=0.060, aug_max_shift=276
  Loss:           cross_entropy
  Batch / epochs: 64 / 50  (early stopping at patience=10 re-enabled)
  Splits:         patient-grouped 80/20 holdout + 5-fold patient-grouped CV

The first trial is seeded with the previously found best (lr=2.465e-3,
dropout=0.0546) so it serves as the immediate baseline.

Usage:
    python -m src.optimize_crosslead_3stage_v2_lrdropout --n-trials 25 --n-folds 5
"""
...
# Fixed at the (48, 96, 192) filter-search winner, with the corrected reg study
# bests for everything else.
FIXED_PARAMS = dict(
...
def objective(trial, X_dev, y_dev, folds):
    ...
    model_params = {**FIXED_PARAMS, "lr": lr, "dropout": dropout}
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna lr+dropout re-tune on (48, 96, 192) architecture"
    )
    ...
    run_dir = Path("optuna_crosslead_3stage_v2_lrdropout") / datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    logger.info("FIXED: %s", {k: list(v) if isinstance(v, tuple) else v for k, v in FIXED_PARAMS.items()})
    ...
    study = optuna.create_study(
        study_name="crosslead_3stage_v2_lrdropout",
        direction="maximize",
        sampler=TPESampler(seed=args.seed),
        storage=storage,
        load_if_exists=True,
    )
    # Seed with the previous best so TPE has an immediate baseline.
    ...
    best_full = {**FIXED_PARAMS, "lr": best["lr"], "dropout": best["dropout"]}
    ...
    lines = [
        "=" * 60,
        "OPTIMIZATION COMPLETE - lr+dropout on (48, 96, 192) architecture",
        "=" * 60,
        f"Best trial:    #{study.best_trial.number}",
        f"Best CV AUROC: {study.best_value:.4f}",
        f"Best lr:       {best['lr']:.6f}",
        f"Best dropout:  {best['dropout']:.4f}",
        "",
        "=" * 60,
        "HOLDOUT TEST",
        "=" * 60,
