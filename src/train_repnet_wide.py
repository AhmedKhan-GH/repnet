"""RepNet Wide — patient-grouped 80/20 + 3-fold CV.

Same training pipeline as src/train_repnet_crosslead.py, but with the wide
4-stage param-efficient model (depthwise-separable convs + lead-attention pool,
NO dilation).

Usage:
    python -m src.train_repnet_wide
    python -m src.train_repnet_wide --n-folds 5 --epochs 50
"""
...
from src.models.repnet_wide import RepNetWide, RepNetWideModel
...
PARAMS = dict(
    stage_filters    = (16, 32, 64, 128),    # slim ladder, peak at 128
    kernels          = (7, 5, 3, 3),         # coarse-to-fine, no dilation
    dropout          = 0.0636,               # transferred from crosslead optuna best
    n_heads          = 4,
    attn_pool_hidden = 64,
    lr               = 8.76e-4,              # transferred
    batch_size       = 64,
...
# ---------------------------------------------------------------------------
# QC + preprocessing + augment+balance (identical to train_repnet_crosslead)
# ---------------------------------------------------------------------------
...
def run_cv(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = RepNetWideModel(**PARAMS, epochs=epochs)
...
# ---------------------------------------------------------------------------
# Test report (identical to train_repnet_crosslead)
# ---------------------------------------------------------------------------
...
def render_architecture(net_params: dict, run_dir: Path) -> str:
    model = RepNetWide(**net_params).eval()
    ...
    print("\n" + "=" * 60)
    print("  Model architecture — RepNet Wide (4-stage, depthwise-separable)")
    print("=" * 60)
    print(text)

    (run_dir / "architecture.txt").write_text(text, encoding="utf-8")

    onnx_path = run_dir / "repnet_wide.onnx"
    try:
        torch.onnx.export(
...
def main():
    parser = argparse.ArgumentParser(description="RepNet Wide — patient-grouped CV")
    parser.add_argument("--data-dir", default="data/seniordesign_upload")
    parser.add_argument("--n-folds",  type=int, default=3)
    parser.add_argument("--epochs",   type=int, default=50)
    args = parser.parse_args()

    run_dir = Path("cv_results") / f"repnet_wide_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    net_params = {k: PARAMS[k] for k in
                  ("stage_filters", "kernels", "dropout", "n_heads", "attn_pool_hidden")}
    ...
    print(f"\n{'#'*60}\n  RepNet Wide — {args.n_folds}-fold patient-grouped CV\n{'#'*60}")
    ...
    torch.save(final_model.model.state_dict(), run_dir / "model_repnet_wide.pt")
    ...
    try:
        ...
        fig.update_layout(title="RepNet Wide — Training Curves",
