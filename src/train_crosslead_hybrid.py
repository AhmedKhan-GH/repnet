"""Train RepNetCrossLeadHybrid (per-lead -> cross-lead attn -> mixed convs -> temporal attn).

Default protocol mirrors the user's preferred eval setup:
  - Full unbalanced dataset (data/seniordesign_upload, ~85/15)
  - Patient-grouped 80/20 holdout + 5-fold CV (no leakage)
  - Weighted CE loss (class-frequency reweighting)
  - lr / dropout fixed at the crosslead Optuna study values
    (optuna/2026-04-17_00-37-13/best_params.json)
  - Basic 3x augmentation, no over/undersampling

Outputs (saved to crosslead_hybrid/<timestamp>/):
  - config.json
  - cv_results.json
  - summary.txt
  - results.log
...
Usage:
    python -m src.train_crosslead_hybrid
    python -m src.train_crosslead_hybrid --no-grouped         # ungrouped splits
    python -m src.train_crosslead_hybrid --data-dir data/seniordesign_upload_balanced
    python -m src.train_crosslead_hybrid --no-temporal-attn   # plain GAP head
"""
...
from src.models.repnet_crosslead_hybrid import RepNetCrossLeadHybridModel
...
# applied to the hybrid architecture as fixed defaults.
HYBRID_PARAMS = dict(
    f1=32,
    f2=64,
    f3=128,
...
def run_cv(params: dict, X_dev, y_dev, folds, epochs: int) -> list[float]:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = RepNetCrossLeadHybridModel(**params, epochs=epochs)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train RepNetCrossLeadHybrid (per-lead bottom, mixed top, temporal attn)",
    )
    ...
    parser.add_argument("--no-temporal-attn", action="store_true",
                        help="Use plain GAP head instead of temporal self-attention")
    args = parser.parse_args()

    grouped = not args.no_grouped
    params = dict(HYBRID_PARAMS)
    if args.no_temporal_attn:
        params["use_temporal_attn"] = False

    run_dir = Path("crosslead_hybrid") / datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    print(f"\n{'#'*72}\n  RepNetCrossLeadHybrid - per-lead -> mixed -> temporal attn\n{'#'*72}")
    ...
    lines = [
        "RepNetCrossLeadHybrid - per-lead bottom, channel-mixed top, temporal attn",
        ...
        f"  use_temporal_attn:{params['use_temporal_attn']}",
        "",
