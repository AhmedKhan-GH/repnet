"""RepNet Efficient Hybrid — 80/20 + 5-fold CV evaluation.

Usage:
    python -m src.train_efficient_hybrid
"""
...
from src.models.repnet_efficient_hybrid import RepNetEfficientHybridModel
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"  RepNet Efficient Hybrid — {args.n_folds}-fold CV")
    print(f"{'#'*60}")
    aurocs = []
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = RepNetEfficientHybridModel(**params)
    ...
    cv = np.array(aurocs)
    print(f"\n{'='*60}")
    print(f"  RepNet Efficient Hybrid")
    print(f"{'='*60}")
