"""RepNet Efficient — 80/20 + 5-fold CV evaluation.

Usage:
    python -m src.train_efficient
"""
...
def main():
    ...
    params = {**EFFICIENT_PARAMS, "epochs": args.epochs}

    print(f"\n{'#'*60}")
    print(f"  RepNet Efficient — {args.n_folds}-fold CV")
    print(f"{'#'*60}")
    aurocs = []
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        model = RepNetEfficientModel(**params)
