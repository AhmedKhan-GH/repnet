"""RepNet CrossLead Deeper — PE-optimised multi-seed retrain on 80/20.

Differences from train_repnet_crosslead_deeper_multiseed.py:

  Augmentation (fixed):
    - Both PE *and* Normal samples are doubled with the same augment pipeline
      (Gaussian noise + random time-shift applied together).  The old script
      only augmented PE, meaning the model could learn "noisy/shifted = PE".
    - The augmentation RNG is seeded from the training seed so runs are
      reproducible.
    - After doubling both classes, majority-undersampling restores 1:1 balance.

  Early stopping (new):
    - Patience-based early stop on *val AUPRC* (not AUROC).  AUPRC weights
      the positive (PE) class heavily; AUROC can be gamed by perfect Normal
      ranking while missing most PE cases.
    - Default patience = 10 epochs; configurable via --patience.

  Seed selection (fixed):
    - Best model across seeds selected by *test AUPRC* (not AUROC).

  Reported metrics:
    - AUROC, AUPRC, Brier (per-seed and aggregate)
    - Sensitivity @ specificity >= 0.80 (sens_sp80) — a direct clinical target
    - Ensemble (mean prob across all seeds) evaluated on all three

Outputs (cv_results/repnet_crosslead_deeper_multiseed_pe_<timestamp>/):
  - config.json, results.log, results.json, summary.txt
  - per_seed_history.json    — train_loss / val_auroc / val_auprc per epoch
  - best_model.pt            — weights from highest-AUPRC seed
...
Usage:
    python -m src.train_repnet_crosslead_deeper_multiseed_pe
    python -m src.train_repnet_crosslead_deeper_multiseed_pe --n-seeds 30 --epochs 80 --patience 12
    python -m src.train_repnet_crosslead_deeper_multiseed_pe --seeds 42 43 44 45 46
"""
...
# ---------------------------------------------------------------------------
# Augmentation (seeded, symmetric across classes)
# ---------------------------------------------------------------------------
...
def augment_balance_train(X: np.ndarray, y: np.ndarray, seed: int):
    """Double both PE and Normal with the same augmentation, then balance.

    Strategy:
      1. Augmented copy = Gaussian noise + random time-shift (seeded).
      2. Both classes doubled: [original, augmented_copy].
      3. Majority-undersample the larger class to restore 1:1 ratio.
      4. Shuffle with the same RNG.

    This ensures the model cannot learn augmentation artefacts as a proxy
    ...
    X_pos_aug = _augment_batch(X_pos, rng, AUG_SIGMA, AUG_MAX_SHIFT)
    X_neg_aug = _augment_batch(X_neg, rng, AUG_SIGMA, AUG_MAX_SHIFT)

    X_combined = np.concatenate([X_neg, X_neg_aug, X_pos, X_pos_aug], axis=0)
...
def train_and_eval_one_seed(X_dev, y_dev, g_dev, X_test, y_test,
    ...
    print(f"  seed={seed}: train={len(y_tr)} (1:1 balanced)  early-stop={len(y_es)}")
...
def main():
    parser = argparse.ArgumentParser(
        description="PE-optimised multi-seed retrain (AUPRC early stop, symmetric augmentation)."
    )
    ...
    run_dir = Path("cv_results") / (
        f"repnet_crosslead_deeper_multiseed_pe_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    )
    ...
    with open(run_dir / "config.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "aug_strategy":   "both_classes_doubled_then_undersample",
    ...
    print(f"\n{'#'*60}")
    print(f"  PE-optimised multi-seed — N={len(seeds)} seeds, 80/20 split")
    ...
    for i, seed in enumerate(seeds):
        ...
        r = train_and_eval_one_seed(
            ...
            seed, args.epochs, args.patience,
        )
    ...
    summary_lines = [
        "",
        "=" * 65,
        f"  PE-optimised multi-seed — N={n} retrains (80/20, AUPRC early stop)",
        "=" * 65,
    ...
    try:
        ...
        fig.update_layout(title=f"Per-seed val AUPRC across {n} seeds",
        ...
        fig.update_layout(
            title=f"Test AUPRC distribution across {n} seeds  "
