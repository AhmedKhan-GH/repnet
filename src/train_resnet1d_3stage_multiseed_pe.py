"""ResNet1D 3-Stage — PE-optimised multi-seed retrain on 80/20.

Same balanced-augmentation paradigm as train_repnet_crosslead_deeper_multiseed_pe.py,
applied to the lighter 3-stage 1D ResNet ablation (~280K params vs 965K for CrossLead).

Augmentation (symmetric):
  - Both PE *and* Normal samples are doubled with Gaussian noise + random time-shift.
  - Augmentation RNG is seeded from the training seed (reproducible).
  - Majority-undersampling restores 1:1 balance after doubling.

Early stopping:
  - Patience-based on val AUPRC (not AUROC).
  - Default patience = 10 epochs; configurable via --patience.

Seed selection:
  - Best model across seeds selected by test AUPRC.

Reported metrics:
  - AUROC, AUPRC, Brier, sens@spec0.80 (per-seed and aggregate)
  - Ensemble (mean prob across all seeds)

Outputs (cv_results/resnet1d_3stage_multiseed_pe_<timestamp>/):
...
Usage:
    python -m src.train_resnet1d_3stage_multiseed_pe
    python -m src.train_resnet1d_3stage_multiseed_pe --n-seeds 30 --epochs 80 --patience 12
    python -m src.train_resnet1d_3stage_multiseed_pe --seeds 42 43 44 45 46
"""
...
class _ResNetPETrainer(ResNet1D3StageModel):
        ...
        for epoch in range(self.epochs):
            ...
            if no_improve >= self.patience:
                print(f"    Early stop at epoch {epoch+1} "
                      f"(no AUPRC improvement for {self.patience} epochs)")
                break
...
def train_and_eval_one_seed(X_dev, y_dev, g_dev, X_test, y_test,
    ...
    model = _ResNetPETrainer(**PARAMS, epochs=epochs, patience=patience, train_seed=seed)
...
def main():
    parser = argparse.ArgumentParser(
        description="ResNet1D-3Stage PE-optimised multi-seed retrain "
                    "(AUPRC early stop, symmetric augmentation)."
    )
    ...
    run_dir = Path("cv_results") / (
        f"resnet1d_3stage_multiseed_pe_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    )
    ...
    print(f"\n{'#'*60}")
    print(f"  ResNet1D-3Stage PE-optimised multi-seed — N={len(seeds)} seeds, 80/20 split")
    ...
    summary_lines = [
        "",
        "=" * 65,
        f"  ResNet1D-3Stage PE multi-seed — N={n} retrains (80/20, AUPRC early stop)",
        "=" * 65,
