"""Receptive-field shape Optuna study for RepNet CrossLead Deeper (3-stage).

Searches only `kernels` (the per-stage temporal kernel tuple) — everything else
fixed at the current best from prior studies. Goal: find whether a different
RF shape moves the CV plateau at ~0.70.

Search space (categorical over tuples)
--------------------------------------
  (3, 3, 3)    RF = 116 ms  (sub-QRS)
  (5, 3, 3)    RF = 144 ms
  (5, 5, 3)    RF = 164 ms  (just-QRS)
  (7, 5, 3)    RF = 180 ms  (CURRENT)
  (9, 5, 3)    RF = 196 ms
  (9, 7, 5)    RF = 292 ms  (full QRS + T onset)
  (11, 7, 5)   RF = 308 ms
  (13, 9, 5)   RF = 356 ms  (~QT interval)

RF formula (3 stages, MaxPool(2) between):
  RF = 2k1 + 4k2 + 8k3 - 13   (samples; ÷250 Hz × 1000 → ms)

Fixed (from prior best studies)
-------------------------------
  Architecture:    stage_filters=(32, 64, 128), n_heads=4
...
Usage:
    python -m src.optimize_crosslead_3stage_rf --n-trials 16 --n-folds 5
    python -m src.optimize_crosslead_3stage_rf --n-trials 24 --n-folds 3   # faster
"""
...
# Fixed at the current best from prior patient-grouped studies.
FIXED_PARAMS = dict(
    ...
    weight_decay  = 1.67e-4,         # corrected reg study best
)

# Fixed augmentation (from corrected reg study best)
AUG_SIGMA     = 0.060
AUG_MAX_SHIFT = 276

# Search space — string keys (Optuna SQLite storage requires JSON-serializable
# types; tuples get coerced to lists, breaking lookup, so we use strings).
KERNEL_CHOICES = ["3-3-3", "5-3-3", "5-5-3", "7-5-3", "9-5-3",
                  "9-7-5", "11-7-5", "13-9-5"]


def _parse_kernels(key: str) -> tuple[int, int, int]:
    a, b, c = key.split("-")
    return int(a), int(b), int(c)


def _rf_samples(kernels: tuple[int, int, int]) -> int:
    """RF for 3 stages of {2 convs + MaxPool(2)}: 2k1 + 4k2 + 8k3 - 13."""
    k1, k2, k3 = kernels
    return 2 * k1 + 4 * k2 + 8 * k3 - 13
...
def objective(
    ...
    kernels_key = trial.suggest_categorical("kernels", KERNEL_CHOICES)
    kernels = _parse_kernels(kernels_key)
    rf_samples = _rf_samples(kernels)
    rf_ms = 1000.0 * rf_samples / 250.0

    model_params = {**FIXED_PARAMS, "kernels": kernels}
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        logger.info(
            "Trial %d kernels=%s | fold %d/%d | AUROC=%.4f",
            trial.number, kernels, fold_idx + 1, len(folds), auroc,
        )

    mean_auroc = float(np.mean(fold_aurocs))
    std_auroc  = float(np.std(fold_aurocs))
    logger.info(
        "Trial %d kernels=%s RF=%dsmp/%.0fms | AUROC=%.4f (+/-%.4f)",
        trial.number, kernels, rf_samples, rf_ms, mean_auroc, std_auroc,
    )
    return mean_auroc
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna RF-shape search for 3-stage CrossLead Deeper"
    )
    ...
    study = optuna.create_study(
        study_name="crosslead_3stage_rf",
        direction="maximize",
    ...
    # Seed the current best (kernels=(7,5,3)) so TPE has a known baseline immediately.
    study.enqueue_trial({"kernels": "7-5-3"})
    ...
    best_kernels = _parse_kernels(study.best_params["kernels"])
    best_full = {**FIXED_PARAMS, "kernels": best_kernels}

    with open(run_dir / "best_params.json", "w") as f:
        json.dump({
            "best_trial":     study.best_trial.number,
            "best_cv_auroc":  study.best_value,
            "best_kernels":   list(best_kernels),
            "best_rf_samples": _rf_samples(best_kernels),
            "best_rf_ms":     1000.0 * _rf_samples(best_kernels) / 250.0,
    ...
    lines = [
        "=" * 60,
        "OPTIMIZATION COMPLETE — RepNet CrossLead Deeper: kernels (RF) search",
        "=" * 60,
        f"Best trial:    #{study.best_trial.number}",
        f"Best CV AUROC: {study.best_value:.4f}",
        f"Best kernels:  {best_kernels}  (RF = {_rf_samples(best_kernels)} samples = "
        f"{1000.0 * _rf_samples(best_kernels) / 250.0:.0f} ms)",
        "",
        "=" * 60,
        "HOLDOUT TEST",
        "=" * 60,
    ...
    for trial in study.trials:
        if trial.value is None:
            continue
        k = _parse_kernels(trial.params["kernels"])
        rf = _rf_samples(k)
        lines.append(
            f"  #{trial.number:3d} | kernels={k} RF={rf:3d}smp/{1000.0*rf/250.0:5.0f}ms "
