"""RepNet CrossLead — CNN with cross-lead attention (spatial attention only).

Architecture:
  1. Per-lead CNN: 12 independent Conv1D streams preserve lead identity
  2. Cross-lead attention: between conv stages, leads attend to each other (spatial)
...
Design rationale:
  ...
  - Temporal dependencies handled by conv layers with large kernels (7, 5)
  - No explicit temporal attention — this variant focuses on spatial (cross-lead) relationships
"""
...
class RepNetCrossLead(nn.Module):
    """CNN with cross-lead attention (spatial attention only).
...
@register_model("repnet_crosslead")
class RepNetCrossLeadModel(BaseModel):
    """Optuna-compatible wrapper around RepNet CrossLead."""
    ...
    def __init__(self, stage_filters=(32, 64), wide_kernel=7, narrow_kernel=5,
        ...
        self.model = None
        logger.info("RepNetCrossLead using device: %s", self.device)
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        self.model = RepNetCrossLead(**self.net_params).to(self.device)
