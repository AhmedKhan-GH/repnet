"""Per-lead 1D Residual CNN — Large variant with 3 conv stages for experimentation.
...
Architecture (per-lead convolutions with simple fusion):
  ...
  Per-lead processing (12 independent streams):
    ...
    Block 3 (F2 → F3, k=3): Conv→BN→ReLU→Conv→BN, proj skip, Add→ReLU→MaxPool(2)→Dropout

  Fusion: Concatenate 12 leads → Conv1D(12*F3→F3, k=1) → GAP → Dropout → Linear(2)

Design choices:
  - Extra layer for increased representational capacity
  - Progressively smaller kernels (7 → 5 → 3) for hierarchical feature extraction
  - Same per-lead architecture as baseline, but deeper
"""
...
class RepNetLarge(nn.Module):
    """Per-lead CNN baseline with 3 conv stages and simple concatenation fusion.
    ...
    def __init__(
        ...
        self.fuse = nn.Sequential(
            nn.Conv1d(n_leads * f3, f3, kernel_size=1),
            nn.BatchNorm1d(f3),
    ...
    def forward(self, x):
        ...
        # Fuse: reshape (B, 12, F3, T') → (B, 12*F3, T') → conv → GAP
        B, L, F, T_out = x.shape
...
@register_model("repnet_baseline_large")
class RepNetBaselineLargeModel(BaseModel):
    """Optuna-compatible wrapper around the RepNet 1D ResNet baseline (large variant)."""

    def __init__(self, stage_filters=(32, 64, 128), wide_kernel=7, narrow_kernel=5,
                 narrow_kernel_2=3, dropout=0.1, lr=1e-3, batch_size=32, epochs=50,
        ...
        self.model = None
        logger.info("RepNetBaselineLarge using device: %s", self.device)

    @staticmethod
    def suggest_params(trial: optuna.Trial) -> dict:
        f1 = trial.suggest_categorical("repnet_large_stage1_filters", [16, 32])
        f2 = trial.suggest_categorical("repnet_large_stage2_filters", [32, 64])
        f3 = trial.suggest_categorical("repnet_large_stage3_filters", [64, 128])
        loss_fn = trial.suggest_categorical(
            "repnet_large_loss_fn", ["cross_entropy", "weighted", "focal"]
        )
        params = {
            "stage_filters": (f1, f2, f3),
            "wide_kernel": trial.suggest_categorical("repnet_large_wide_kernel", [5, 7, 9]),
            "narrow_kernel": trial.suggest_categorical("repnet_large_narrow_kernel", [3, 5]),
            "narrow_kernel_2": trial.suggest_categorical("repnet_large_narrow_kernel_2", [3, 5]),
            "dropout": trial.suggest_float("repnet_large_dropout", 0.05, 0.4),
            "lr": trial.suggest_float("repnet_large_lr", 1e-4, 5e-3, log=True),
            "batch_size": trial.suggest_categorical("repnet_large_batch_size", [32, 64]),
            "epochs": trial.suggest_int("repnet_large_epochs", 10, 50),
            "loss_fn": loss_fn,
        }
        if loss_fn == "focal":
            params["focal_gamma"] = trial.suggest_float("repnet_large_focal_gamma", 0.5, 5.0)
            params["focal_alpha"] = trial.suggest_float("repnet_large_focal_alpha", 0.1, 0.9)
        return params
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        self.model = RepNetLarge(**self.net_params).to(self.device)
