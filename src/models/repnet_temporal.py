"""RepNet Temporal — CNN with temporal self-attention within each lead.
...
Architecture:
  ...
  Stage 1: 12x Conv1D(1→F1, k=7) → TemporalSelfAttention(per-lead, over time)
  Stage 2: 12x Conv1D(F1→F2, k=5) → TemporalSelfAttention(per-lead, over time)
  Fuse:    Concatenate 12 leads → Conv1D(12*F2→F_out, k=1) → GAP → Dropout → Linear(2)

Design rationale:
  - Temporal self-attention captures long-range dependencies within each lead
  - Important for ECG: P-wave to QRS to T-wave relationships, ST-segment changes
  - Each lead processes independently (no cross-lead attention)
  - Better for capturing classic ECG PE signs (T-wave inversions, QRS prolongation)
"""
...
class RepNetTemporal(nn.Module):
    """CNN with temporal self-attention within each lead.

    Preserves lead identity throughout — each lead processes independently
    with temporal self-attention to capture long-range dependencies.
    """
...
@register_model("repnet_temporal")
class RepNetTemporalModel(BaseModel):
    """Optuna-compatible wrapper around RepNet Temporal."""
    ...
    def __init__(self, stage_filters=(32, 64), wide_kernel=7, narrow_kernel=5,
        ...
        self.model = None
        logger.info("RepNetTemporal using device: %s", self.device)

    @staticmethod
    def suggest_params(trial: optuna.Trial) -> dict:
        f1 = trial.suggest_categorical("temporal_stage1_filters", [16, 32])
        f2 = trial.suggest_categorical("temporal_stage2_filters", [32, 64])
        loss_fn = trial.suggest_categorical(
            "temporal_loss_fn", ["cross_entropy", "weighted", "focal"]
        )
        params = {
            "stage_filters": (f1, f2),
            "wide_kernel": trial.suggest_categorical("temporal_wide_kernel", [5, 7, 9]),
            "narrow_kernel": trial.suggest_categorical("temporal_narrow_kernel", [3, 5]),
            "dropout": trial.suggest_float("temporal_dropout", 0.05, 0.4),
            "n_heads": trial.suggest_categorical("temporal_n_heads", [2, 4]),
            "lr": trial.suggest_float("temporal_lr", 1e-4, 5e-3, log=True),
            "batch_size": trial.suggest_categorical("temporal_batch_size", [32, 64]),
            "epochs": trial.suggest_int("temporal_epochs", 10, 50),
            "loss_fn": loss_fn,
        }
        if loss_fn == "focal":
            params["focal_gamma"] = trial.suggest_float("temporal_focal_gamma", 0.5, 5.0)
            params["focal_alpha"] = trial.suggest_float("temporal_focal_alpha", 0.1, 0.9)
        return params
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        self.model = RepNetTemporal(**self.net_params).to(self.device)
