...
Figure~\ref{fig:arch} shows the data flow. The visual convention: \textbf{stacked boxes denote per-lead operations} (the same weights applied independently to all 12 leads, no cross-lead interaction), while \textbf{single wide boxes denote cross-lead operations} where information flows between leads.
...
\begin{tikzpicture}[
    ...
    boxw/.style   ={draw, rectangle, rounded corners=1mm, minimum height=8mm, minimum width=46mm, align=center, font=\small\sffamily},
    attn/.style   ={boxw, fill=orange!18},
    fuse/.style   ={boxw, fill=green!15},
    io/.style     ={boxw, fill=gray!12, font=\small\sffamily\bfseries, minimum width=46mm},
    dim/.style    ={font=\scriptsize\itshape, midway, right=2mm, text=gray!50!black},
    convback/.style ={draw, fill=blue!6,  rounded corners=1mm, minimum height=8mm, minimum width=46mm},
    convmid/.style  ={draw, fill=blue!10, rounded corners=1mm, minimum height=8mm, minimum width=46mm},
    convfront/.style={draw, fill=blue!16, rounded corners=1mm, minimum height=8mm, minimum width=46mm, align=center, font=\small\sffamily},
]

% Helper: a "per-lead stack" rendered as 3 offset rectangles. Use \perleadstack{name}{label}.
\def\perleadstack#1#2{
    \node[convback]  (#1-b2) {};
    \node[convmid,   xshift=1.2mm, yshift=-1.2mm] at (#1-b2.center) (#1-b1) {};
    \node[convfront, xshift=1.2mm, yshift=-1.2mm] at (#1-b1.center) (#1) {#2};
}

\node[io] (in) {ECG input \\ \scriptsize $(B,\, 12 \text{ leads},\, 2500 \text{ samples})$};

% Stage 1
\node[below=10mm of in] (anchor1) {};
\begin{scope}[shift={(anchor1)}]
\perleadstack{c1}{PerLeadConv $1{\to}48,\, k{=}7$ \\ \scriptsize\itshape (shared weights, applied per lead)}
\end{scope}
\node[attn, below=10mm of c1] (a1) {CrossLeadAttn — 48 ch, 4 heads \\ \scriptsize\itshape (mixes the 12 lead-tokens)};

% Stage 2
\node[below=10mm of a1] (anchor2) {};
\begin{scope}[shift={(anchor2)}]
\perleadstack{c2}{PerLeadConv $48{\to}96,\, k{=}5$ \\ \scriptsize\itshape (shared weights, applied per lead)}
\end{scope}
\node[attn, below=10mm of c2] (a2) {CrossLeadAttn — 96 ch, 4 heads \\ \scriptsize\itshape (mixes the 12 lead-tokens)};

% Stage 3
\node[below=10mm of a2] (anchor3) {};
\begin{scope}[shift={(anchor3)}]
\perleadstack{c3}{PerLeadConv $96{\to}192,\, k{=}3$ \\ \scriptsize\itshape (shared weights, applied per lead)}
\end{scope}
\node[attn, below=10mm of c3] (a3) {CrossLeadAttn — 192 ch, 4 heads \\ \scriptsize\itshape (mixes the 12 lead-tokens)};

% Fusion head
\node[fuse, below=10mm of a3] (cat) {Concat 12 leads $\to$ Conv1d $1{\times}1\ (2304{\to}192)$};
\node[fuse, below=of cat]     (gap) {GAP $\to$ Dropout};
...
% Stage groupings
\node[draw, dashed, rounded corners=2mm, fit=(c1-b2)(a1), inner sep=2mm,
      label={[font=\footnotesize, gray!50!black]right:Stage 1}] {};
\node[draw, dashed, rounded corners=2mm, fit=(c2-b2)(a2), inner sep=2mm,
      label={[font=\footnotesize, gray!50!black]right:Stage 2}] {};
\node[draw, dashed, rounded corners=2mm, fit=(c3-b2)(a3), inner sep=2mm,
      label={[font=\footnotesize, gray!50!black]right:Stage 3}] {};
\node[draw, dashed, rounded corners=2mm, fit=(cat)(gap)(fc), inner sep=2mm,
      label={[font=\footnotesize, gray!50!black]right:Fusion head}] {};

\end{tikzpicture}
\caption{Architecture of the 3-stage RepNet CrossLead Deeper. \textbf{Stacked blue boxes} denote per-lead operations: the same convolutional kernel is applied independently to each of the 12 leads (weight-shared, no cross-lead information flow). \textbf{Single orange/green boxes} denote operations with cross-lead information flow. CrossLeadAttention is the only place inter-lead mixing happens within the stages: each lead's feature map is pooled to a single token, attention runs over the 12 lead-tokens, the output gates the full-resolution feature maps multiplicatively (preserving lead identity). The fusion head concatenates the 12 leads only at the very end. Dimension annotations show $(B, L, C, T)$; the temporal axis halves at each stage's MaxPool$(2)$.}
\label{fig:arch}
\end{figure}
