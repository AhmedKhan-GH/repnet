...
Table \_(3) MUSE Implementations
	...
	In response to this, we briefly considered using an alternative library.  During one of our meetings, ECGDataKit was recommended, as its functions may be more similar to MUSE than Neurokit. To test this, we compared the median R-Peak Amplitude where ECGDataKit did give closer results. However, while the functions of the library accepted all 500 Hz ECGs, it refused to work with 250 Hz ECGs, giving an error in return. It also provided less functions for finding other features outside of cleaning, filtering, and R-Peak Detection.   
Neurokit remained the better option, as although it did not seem reliable in extracting features from raw ECG waveforms, after passing the extracted featurization datasets into a random forest model, we get a clearer understanding. The setup of the model consists of a Stratified Group K Fold to split the dataset into test and training sets based on an obfuscated patient MRN, which prevents data leakage. The results displayed in Table \_(4) reveal that across both the unbalanced and balanced datasets that the Neurokit and MUSE featurizations perform similarly. This enforces the idea that although Neurokit implementations underreport values, it does it consistently which does not affect the model too greatly. 
...
**Other Attempts**  
A topic of interest had been using results from other conditions to find patterns for preeclampsia. For example, a common criteria for left ventricular hypertrophy (LVH) when diagnosing via an ecg is a R wave in AVL above 11 mm (1.1 mV)\[Left Ventricular Hypertrophy (LVH)\]. However, while the average for Preeclampsia was higher than Normal’s, when looking at the max values, Normal prevailed. In fact, the max was 2.3 mV, showing that there is little correlation between AVL and preeclampsia. For additional confirmation, the other tests for LVH is a R wave in aVF and/or a high S wave in aVR. Neither showed any promising results, with Normal and Preeclampsia being similar. 
