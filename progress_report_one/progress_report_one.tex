...
We aim to bolster clinical interpretability through attention mapping of the neural network. And we aim to train a random forrest (RF) classifier and visualize manually extracted ECG features from the RF classifier alongside the attention mapping of deep learning to augment clinical workflows by providing explainability to clinicians, assinging quantities to suggestions by the model. And we intend to create a machine learning visualization tool to show model prediction, attention mapping, and featurization suggestions in real time.
...
ECG feature analysis and feature extraction as a field will be an important aspect of our explainable RF classifier. Kaplan et al. as a review of ECG feature analysis, going into the anatomy of a single heartbeat and naming everything we should be familiar with, and also exploring applications of machine learning [cite]. We selected Singh et al. as a review of ECG feature extraction, it goes over the very algorithms that we end up using in existing libraries such as Neurokit2 [cite].
...
\section{Preeclampsia RF Original Studies}

\subsection{Previous Work}

Decision tree-based models have proven to be successful in predicting preeclampsia, though none through the use of ECGs for inference. Bulez et al observed the use of a LightGBM model using data from blood tests, blood pressure, and other factors from patient EHRs with the conclusion that age and features from the blood test were most important for inference \cite{bulez}. In Zhang et al's work, the XGBoost model was used on urine samples to determine seven compounds that were most important in predicting preeclampsia \cite{zhang}.
...
\section{Preeclampsia CNN Original Studies}
...
To achieve optimal model performance with the CNN Transformer we will use ECG-AI as a baseline architecture and experiment with additional layers and hyperparameters using the Optuna framework. We will also implement other papers' transformer-based models shown to perform well on similar ECG classification datasets. This requires making modular components — models with exposed hyperparameter inputs and interchangeable preprocessing and data augmentation pipelines supporting majority undersampling, SMOTE, or GAN strategies.
...
\midrule
\multicolumn{3}{l}{\textit{Preeclampsia RF Original Studies}} \\
