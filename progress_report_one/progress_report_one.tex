...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{12911_2023_2233_Fig1_HTML}
  \caption{Example 12-lead ECG recording over 10 seconds showing all 12 waveforms (I, II, III, aVR, aVL, aVF, V1--V6). Each lead captures the heart's electrical activity from a different spatial angle.}
  \label{fig:ecg-12lead-example}
\end{figure}
...
A high Frobenius norm ($>2.0$) would indicate that preeclampsia significantly alters how leads correlate with each other, meaning the cross-lead relationship itself is a discriminative feature best captured by 2D convolution across leads. Our value of 1.026 indicates only modest disruption --- the leads correlate roughly the same way in both classes. The discriminative information therefore resides within each lead's waveform, not in inter-lead coupling, justifying per-lead 1D convolution (channel-independent processing). [cite frobenius norm
...
\node[out] (out_r) at (0, -4.4) {GAP $\to$ Dense(2)};
...
\node[out] (out_r) at (0, -6.1) {GAP $\to$ Dense(2)};
\draw[arrow] (fuse) -- (out_r);
...
\end{figure}
 ...
 Initial experiments indicated a slight increase in performance over the baseline, so we then performed a broad Optuna hyperparameter study optimizing learning rate and dropout rate over 20 trials that took a few hours to complete overnight. This gives us an estimate for the combinatorial explosion of larger training jobs, and how we should allocate our time and compute resources accordingly. The below contour plot that the model benefits from lower learning rates universally, with some slight tendency towards lower dropout rate, but that goes against empirical results from papers so further experiment will be necessary.
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{contour_plot.png}
\caption{Optuna contour plot of the hyperparameter search over learning rate and dropout for RepNet Hybrid. The objective value (mean 5-fold CV AUROC) is highest in the low-dropout ($<0.1$), moderate learning rate ($\sim 8 \times 10^{-4}$) region. High dropout ($>0.25$) consistently degrades performance.}
\label{fig:optuna-contour}
\end{figure}

\begin{algorithm}[H]
\caption{Optuna TPE Hyperparameter Study on RepNet Hybrid}\label{alg:optuna}
\SetAlgoLined
\KwIn{$X, y$ (full dataset), $N$ (number of trials), $K=5$ (folds)}
\KwOut{Best model weights, test AUROC}
\textbf{Fixed:} filters $= (32, 64)$, $k = (7, 5)$, heads $= 4$, batch $= 64$, epochs $= 50$, loss $=$ weighted CE\;
$X_{\text{dev}}, X_{\text{test}}, y_{\text{dev}}, y_{\text{test}} \gets \text{StratifiedSplit}(X, y, 80/20)$\;
$\text{folds} \gets \text{StratifiedKFold}(X_{\text{dev}}, y_{\text{dev}}, K)$\;
\BlankLine
\For{$t \gets 1$ \KwTo $N$}{
    $\text{lr} \sim \text{LogUniform}(10^{-4},\; 5 \times 10^{-3})$\;
    $\text{dropout} \sim \text{Uniform}(0.05,\; 0.4)$\;
    \BlankLine
    \For{$k \gets 1$ \KwTo $K$}{
        $X_{\text{tr}}, X_{\text{val}} \gets \text{folds}[k]$\;
        $X_{\text{tr}} \gets \text{Augment}(X_{\text{tr}},\; \sigma{=}0.02,\; \text{scale}{=}0.1,\; \text{shift}{=}100)$\;
        $\mathcal{M} \gets \text{RepNetHybrid}(\text{lr}, \text{dropout}, \textbf{Fixed})$\;
        $\text{Train}(\mathcal{M}, X_{\text{tr}}, X_{\text{val}}, \text{patience}{=}10)$\;
        $A_k \gets \text{AUROC}(\mathcal{M}, X_{\text{val}})$\;
    }
    Report $\frac{1}{K}\sum_{k=1}^{K} A_k$ to TPE sampler\;
}
\BlankLine
$\text{lr}^*, \text{dropout}^* \gets \text{best trial parameters}$\;
$\mathcal{M}^* \gets \text{RepNetHybrid}(\text{lr}^*, \text{dropout}^*, \textbf{Fixed})$\;
$\text{Train}(\mathcal{M}^*, X_{\text{dev}}, \text{patience}{=}10)$\;
\Return $\text{AUROC}(\mathcal{M}^*, X_{\text{test}})$\;
\end{algorithm}
...
PE ECGs & 187 & 337 \\
\midrule
\textbf{Total ECGs} & \textbf{369} & \textbf{2,191} \\
Ratio (Normal:PE) & $\sim$1:1 & $\sim$5.5:1 \\
\bottomrule
\end{tabular}
\end{table}
