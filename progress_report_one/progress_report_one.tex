...
To our knowledge there is only one relevant previous work conducting a deep learning study on preeclampsia ECG waveforms, Butler et al. which presents a 1-D CNN they call ECG-AI \cite{butler_ai-based_2024}. There is also an older study by Euliano et al. which is on 3-Lead instead of 12-Lead ECG and they perform linear discriminant analysis (LDA), a classical ML approach. The paper lends credence to a reduced lead input \cite{euliano_photoplethysmography_2018}.

\subsection{ECG Classification Literature Reviews}
In the general field of ECG Waveform classification, we finished reviewing a literature review by Boulif et al., which is a broad scope review that covers both machine learning and deep learning methods \cite{boulif_literature_2023}. We also selected Prakash et al. for future reading, it is a deep systematic review that covers more research process and model architecture \cite{jaya_prakash_advances_2025}.

Focusing on deep learning we have finished reviewing M. Ansari et al. for its comprehensive survey of transformers and LLMs specifically applied to ECG classification \cite{ansari_survey_2025}. Furthermore we have also finished reviewing Y. Ansari et al. for its deep systematic analysis and comparison of DL and transformer model architectures for ECG comparison \cite{ansari_deep_2023}. They both show the prevalence of convolutional neural networks, some enhanced by attention mechanisms.
...
$\checkmark$ & Euliano et al.~\cite{euliano_photoplethysmography_2018} & Original & ECG and photoplethysmography features for preeclampsia diagnosis \\
\midrule
...
$\checkmark$ & Xue et al.~\cite{xue_does_2024} & Original & DL interpretation from reduced ECG lead sets vs.\ full 12-lead \\
$\checkmark$ & Attia et al.~\cite{attia_automated_2022} & Original & AI detection of low ejection fraction from a single-lead ECG \\
\midrule
\multicolumn{4}{l}{\textit{Random Forest Featurization Progress}} \\
$\checkmark$ & Bulez et al.~\cite{bulez_artificial_2024} & Original & LightGBM on EHR data for early preeclampsia diagnosis \\
$\checkmark$ & Zhang et al.~\cite{zhang_development_2023} & Original & XGBoost on urine metabolomics for preeclampsia prediction \\
$\checkmark$ & Scherer et al.~\cite{scherer_abnormal_2009} & Original & T-wave axis association with coronary artery calcification \\
\midrule
...
\multicolumn{4}{l}{\textit{ECG Waveform Preprocessing Reviews}} \\
 ...
 & Satija et al.~\cite{satija_review_2018} & Review & Overview of ECG signal quality assessment techniques \\
\midrule
\multicolumn{4}{l}{\textit{Preeclampsia ECG Waveform Features}} \\
 & Angeli et al.~\cite{angeli_novel_2015} & Original & Review of ECG features associated with preeclampsia \\
 & Raffaelli et al.~\cite{raffaelli_pre-eclampsia_2014} & Original & QT dispersion and ventricular repolarization in preeclampsia \\
\midrule
\multicolumn{4}{l}{\textit{ECG Feature Analysis Literature Reviews}} \\
 & Kaplan et al.~\cite{kaplan_berkaya_survey_2018} & Review & General overview of ECG feature analysis \\
 & Singh et al.~\cite{singh_ecg_2023} & Review & General overview of ECG feature extraction \\
\midrule
\multicolumn{4}{l}{\textit{Data Augmentation for ECG Waveforms}} \\
 & Rahman et al.~\cite{rahman_systematic_2023} & Review & Overview of ECG data augmentation strategies \\
 & Xia et al.~\cite{xia_generative_2023} & Original & Transformer GAN for ECG data augmentation \\
\midrule
\multicolumn{4}{l}{\textit{Preeclampsia Decision Tree Classification}} \\
 & Park et al.~\cite{park_pchd_2014} & Original & Personalized decision tree for heartbeat type classification \\
 & Musa et al.~\cite{musa_predicting_2023} & Original & Decision tree with PCA for preeclampsia detection \\
\midrule
\multicolumn{4}{l}{\textit{ECG Model Efficiency and Compression}} \\
 & Keivanimehr et al.~\cite{keivanimehr_tinyml_2025} & Review & Survey of TinyML and edge intelligence for ECG monitoring \\
 & Gupta et al.~\cite{gupta_comprehensive_2024} & Review & Overview of efficient architectures for ECG classification \\
\bottomrule
\end{tabular}
\end{table*}
