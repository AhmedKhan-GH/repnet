...
\begin{frame}{Decision-Tree Modeling --- Heatmap}
  \centering
  \vspace{-0.3em} % Pulls content up to utilize top margins
  {\small\Raleway\bfseries\color{aggieblue} Feature Selection by Methods --- 12 Leads}\\[0.4em]
  
  % Centers the wide heatmap evenly into both left/right margins
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.08\textwidth,height=5.2cm,keepaspectratio]{Heatmap12leads.png}
  }
  
  \vspace{-0.5em} % Controlled vertical space between heatmap and bottom row
  
  % Split bottom area: Text on the left, Legend pushed to the right
  \makebox[\textwidth][c]{%
    \begin{minipage}[c]{0.35\textwidth}
      \begin{itemize}
        \item QRS Intrinsicoid
        \item T-pe Interval
      \end{itemize}
    \end{minipage}%
    \hfill % <--- This dynamically pushes the legend to the rightmost limit of the bounding box
    \begin{minipage}[c]{0.60\textwidth}
      \flushright % <--- This forces the image to align to the right inside its own box
      % Legend scaled to fit the right-side allocation beautifully
      \includegraphics[width=1.1\textwidth,height=2.7cm,keepaspectratio]{heatmaplegend.png}
    \end{minipage}
  }
\end{frame}
...
\begin{frame}{Conclusion}
  {\small\Raleway\bfseries\color{aggieblue} Key Findings}\par\vspace{0.4em}
  \usebeamerfont{footline}\color{coolgray}
  \begin{itemize}
    ...
    \item Decision-tree ensemble (\textbf{Unweighted Voting+}) achieves \textbf{AUROC $75.72 \pm 3.94$} with a \textbf{balanced accuracy $68.50 \pm 3.22$} --- stable, interpretable baseline built on 175 features
    \item RepNet-SE achieves \textbf{AUROC 0.787} and \textbf{AUPRC 0.362} (5-bag ensemble, best of 20 seeds) with \textbf{58.2\% sensitivity at 80\% specificity}, using only 65K parameters on raw ECG waveforms
