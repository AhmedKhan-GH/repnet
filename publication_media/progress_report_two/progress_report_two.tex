...
Quality filtering removes 8 recordings with flat leads (std $< 10^{-4}$) and 5 with missing patient identifiers, yielding 2{,}178 usable recordings (335 PreE, 1{,}843 Normal).
...
Figures~\ref{fig:saliency1}--\ref{fig:saliency2} show all 12 leads ranked by total attribution magnitude. Red regions indicate features that push the prediction toward PreE; blue regions push toward Normal. The model consistently highlights R-peak onsets and the ST--T segment across multiple heartbeat cycles. Leads V6 and V3 show the strongest attributions, with the ST segment and T-wave morphology receiving the most attention. The attribution pattern is periodic, tracking individual heartbeats rather than responding to a single anomalous region, suggesting the model aggregates evidence across multiple cardiac cycles.
