...
\node[branch, below=1.0cm of input, xshift=-2.7cm] (b1) {DW $k\!=\!5$};
...
\end{scope}
\coordinate (descleft) at ([xshift=-8.7cm]s1bg.west);
\node[stagelabel, anchor=north west] at ([yshift=-4pt]descleft |- s1bg.north)
  (s1label) {Stage 1 \textcolor{textmid}{\normalfont\small — Multi-Scale Inception + SE + Attention}};
\node[anchor=north west, text width=7.5cm, font=\large\sffamily, text=textmid,
      align=left] at ([yshift=-1pt]s1label.south west)
  {Parallel depthwise convolutions at three scales ($k$=5, 9, 15)
   capture QRS and P/T-wave morphology simultaneously.
   SE recalibrates channel importance before cross-lead
   attention models inter-lead dependencies.};
...
\end{scope}
\node[stagelabel, anchor=north west] at ([yshift=-4pt]descleft |- s2bg.north)
  (s2label) {Stage 2 \textcolor{textmid}{\normalfont\small — SE Per-Lead Block + Cross-Lead Attention}};
\node[anchor=north west, text width=7.5cm, font=\large\sffamily, text=textmid,
      align=left] at ([yshift=-1pt]s2label.south west)
  {Two stacked depthwise-separable convolutions deepen per-lead
   representations with a residual skip connection. SE recalibration
   and 4-head cross-lead attention are applied after pooling.};
...
\end{scope}
\node[stagelabel, anchor=north west] at ([yshift=-4pt]descleft |- s3bg.north)
  (s3label) {Stage 3 \textcolor{textmid}{\normalfont\small — SE Per-Lead Block + Cross-Lead Attention}};
\node[anchor=north west, text width=7.5cm, font=\large\sffamily, text=textmid,
      align=left] at ([yshift=-1pt]s3label.south west)
  {Same structure as Stage~2 at higher channel capacity. The final
   64-dimensional per-lead features encode the richest cardiac
   morphology before fusion.};
...
\end{scope}
\node[stagelabel, anchor=north west] at ([yshift=-4pt]descleft |- fusbg.north)
  (fuslabel) {Classification Head \textcolor{textmid}{\normalfont\small — Lead Attention Pool + Linear}};
\node[anchor=north west, text width=7.5cm, font=\large\sffamily, text=textmid,
      align=left] at ([yshift=-1pt]fuslabel.south west)
  {Global average pooling collapses each lead's temporal dimension.
   A learned attention mechanism weights the 12 leads by
   per-sample importance, then a linear layer produces the
