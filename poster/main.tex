...
\begin{column}{\colwidth}
  ...
  \begin{tabular}{ll | ll}
    ...
    \midrule
    $P_{\max}$ & $\max_{\ell}(P_{\text{off}}^{\ell} - P_{\text{on}}^{\ell})$ & $T_{pe}/\text{QT}$ & $T_{pe} / (T_{\text{off}} - R_{\text{on}})$ \\
    $P_{\min}$ & $\min_{\ell}(P_{\text{off}}^{\ell} - P_{\text{on}}^{\ell})$ & $T_{pe}/\text{QTc}$ & $(T_{pe} / \text{QT}) \cdot \sqrt{\Delta R_{pk}}$ \\
    P-Wave Disp. & $P_{\max} - P_{\min}$ & ST Elev./Dep. & $\mathbf{1}[>50\%\ \text{ST shift}]$ \\
    QT Disp. & $\text{QT}_{\max} - \text{QT}_{\min}$ & Amp. Diff. & $A_{\text{ST}} - A_{\text{PR}}$ \\
    $T_{pe}$ & $(T_{\text{off}} - T_{\text{pk}})$ & QRS-T Angle & $|\text{QRS Ax.} - \text{T Ax.}|$ \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
