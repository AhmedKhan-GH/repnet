...
\begin{frame}{Neural Network Modeling --- Multi-Head Cross-Lead Attention}
  \begin{columns}[T]
    ...
    \begin{column}{0.38\textwidth}
      {\small\Raleway\bfseries\color{aggieblue} How It Works}\\[0.4em]
      \usebeamerfont{footline}\color{coolgray}
      Applied \textbf{after every conv stage} so the model learns cross-lead dependencies at multiple abstraction levels.\\[0.5em]

      \begin{enumerate}\usebeamerfont{footline}\color{coolgray}
        \item \textbf{Pool} each lead's $(C, T)$ map to a $C$-dim token
        \item \textbf{Attend} across 12 lead tokens with 4-head self-attention + residual + LayerNorm
        \item \textbf{Gate} $\sigma(\text{FC})$ produces per-lead weights $\in [0,1]$
        \item \textbf{Scale} original feature maps by learned gates
      \end{enumerate}

      \vspace{0.5em}
      {\usebeamerfont{footline}\color{coolgray}
      Final fusion: \textbf{LeadAttentionPool} --- learned softmax weighting over 12 lead embeddings $\to$ single patient vector.}
    \end{column}
